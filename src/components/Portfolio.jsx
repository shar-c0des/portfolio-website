...
const PROJECTS = [
  {
    ...
    year: "2025",
    image: homeImg,
    details: "Academic research project with a team where we identified a gap in visualizing 2D interior designs in real spaces. Built this AR app as part of our research on optimizing AR performance on Android devices. The work contributed to our dissertation on AR usability.",
    stack: ["Flutter", "Dart", "ARCore", "Firebase"],
    github: "https://github.com/shar-c0des/Roomantics-AR-App",
    demoVideo: demoVideo,
    researchProposal: `${import.meta.env.BASE_URL}literatureReview.pdf`,
  },
  {
    id: 2,
    title: "PulseBuy",
    ...
    year: "2025",
    image: pulsebuyImg,
    details: "Analyzed Taobao and Gmarket to understand what makes e-commerce work, then built a full C2C marketplace with multi-role dashboards, seller analytics, and comprehensive product management. Docker-powered with PHP/MySQL backend - from authentication to order processing, everything an e-commerce platform needs.",
    stack: ["PHP", "MySQL", "Docker", "HTML", "CSS"],
    github: "https://github.com/shar-c0des/PulseBuy",
    demo: "https://shar-c0des.github.io/PulseBuy-demo",
    researchProposal: `${import.meta.env.BASE_URL}projectProposal.pdf`
  },
  {
    id: 3,
    ...
    details: "My way of helping people find the right book for their mood. Started because I kept recommending books to friends and realized there should be a smarter way to match readers with books based on what they're feeling or needing right now.",
...
const WORKFLOW = [
  { step: "01", name: "BREAK IT DOWN", desc: "I break big problems into small, manageable pieces that I can actually solve one by one." },
  { step: "02", name: "BUILD THE FOUNDATION", desc: "First version that works, then I make it better. Never perfect on the first try." },
  { step: "03", name: "ITERATE UNTIL IT FEELS RIGHT", desc: "Test it, get feedback, fix what sucks, repeat. Good enough is rarely good enough." }
];
  ...
  overflow: hidden;
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? 'py-4 bg-black/80 backdrop-blur-xl border-b border-white/5' : 'py-8 bg-transparent'}`}>
        <div className="max-w-screen-2xl mx-auto px-8 md:px-12 flex justify-between items-center">
          ...
          <div className="flex space-x-10 text-[11px] font-medium tracking-widest text-zinc-400">
            ...
            <div className="md:col-span-8 z-10">
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                <div className="text-zinc-600 mb-8">
                  <span className="text-[10px] font-mono tracking-[0.4em] uppercase">Highly Into Tech | Building stuff</span>
                </div>
        ...
        <section id="contact" className="py-60 px-8 md:px-12 border-t border-white/5 text-center relative overflow-hidden">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
            ...
            <div className="flex flex-col md:flex-row items-center justify-center space-y-6 md:space-y-0 md:space-x-12 mb-20 relative z-10">
              <a 
                href="mailto:ngomakapilesharleen703@gmail.com" 
                className="text-xl md:text-2xl font-light hover:text-zinc-500 transition-colors border-b border-zinc-900 pb-1 relative z-10 cursor-pointer"
                style={{ pointerEvents: 'auto' }}
              >
                ngomakapilesharleen703@gmail.com
              </a>
              <div className="w-2 h-2 rounded-full bg-zinc-800 hidden md:block"></div>
              <span className="text-zinc-600 font-mono text-[10px] tracking-[0.3em] uppercase italic">IT ALL. STARTS HERE.</span>
            </div>
            ...
            <div className="max-w-7xl w-full grid lg:grid-cols-2 gap-12 items-center">
              ...
              >
                ...
                <div className="pt-8 border-t border-white/5 space-y-4">
                  ...
                  {selectedProject.researchProposal && selectedProject.researchProposal !== "#" && (
                    <a 
                      href={selectedProject.researchProposal} 
                      target="_blank" 
                      rel="noreferrer"
                      className="flex items-center space-x-6 text-white hover:text-zinc-500 transition-all group"
                    >
                      <span className="text-2xl font-bold tracking-tighter">
                        {selectedProject.title === "PulseBuy" ? "PROJECT PROPOSAL" : "RESEARCH PROPOSAL"}
                      </span>
                      <ArrowRight size={28} className="group-hover:translate-x-4 transition-transform" />
                    </a>
                  )}
                  
                  {/* Show COMING SOON button for projects in progress or without demo */}
                  {(!selectedProject.demo || selectedProject.category.includes("In Progress")) && (
                    <div className="flex items-center space-x-6 text-zinc-400 cursor-not-allowed">
                      <span className="text-2xl font-bold tracking-tighter">
                        <span className="animate-pulse">COMING SOON</span>
                      </span>
                      <div className="w-7 h-7 border-2 border-zinc-400 rounded-full animate-pulse"></div>
                    </div>
                  )}
                </div>
              </motion.div>
